...
/**
 * 三品种窗口均价（发改委口径：布伦特×40% + 迪拜×30% + 米纳斯×30%）
 * 数据源优先级：FRED Platts现货（主）→ Yahoo BZ=F 基差修正填充近期缺口 → AV → EIA
 * Dubai = Brent - 8.5（当前现货价差），Minas = Dubai - 3.2（民间拟合公式）
 */

// 调价日次日（下一个工作日）：一揽子均价统计起点
...
// v20260818-1: basket window ends at today (not yesterday) so the cycle starts after last adjust
...
async function fetchWindowAvg(
  ...
  console.log(`[窗口] v2 fetchWindowAvg called, lastAdjustDate=${lastAdjustDate || 'empty'}`);
  ...
  // ── 1. 并行拉：FRED 现货(主) + Yahoo BZ=F 期货(基差补充) + AV(备用) ──────
  //  发改委口径：普氏新加坡现货评估价，FRED DCOILBRENTEU 即为该口径
  //  Yahoo BZ=F 为 ICE 期货，当前较现货高约 $4-5，需做基差修正后才能用于近期缺口填充
  const [fredBrentRes, avBrentRes, yahooBzRes] =
    await Promise.allSettled([
      fetchFredSeries("DCOILBRENTEU", yahooStart), // 取更早一段以便计算基差
      fetchAvDaily("BRENT", windowStart),
      fetchYahooHistory("BZ=F", yahooStart),
    ]);

  const brentMap = new Map<string, number>(); // 日期 → Platts 现货 Brent

  // ① FRED — Platts Dated Brent 现货，主源（权威口径，3-5天滞后）
  const fredRows: Array<{ period: string; val: number }> = [];
  if (fredBrentRes.status === "fulfilled") {
    for (const r of fredBrentRes.value) {
      fredRows.push(r);
      if (r.period >= windowStart) brentMap.set(r.period, r.val);
    }
    console.log(`[窗口] FRED Brent 现货: ${brentMap.size} 天 in window`);
  } else console.warn("[窗口] FRED Brent 失败:", (fredBrentRes.reason as Error)?.message);

  // ② AV 补充（FRED 未覆盖的日期，同为现货口径）
  ...
  // ③ Yahoo BZ=F 基差修正 — 计算期货与现货的价差（basis），用于填充 FRED 滞后的近期缺口
  //    发改委口径：现货价 = Yahoo期货 + basis（basis通常为负，期货溢价于现货约 $4-5）
  if (yahooBzRes.status === "fulfilled") {
    const yahooMap = new Map(yahooBzRes.value.map(r => [r.period, r.val]));
    // 计算最近 5 个 FRED 有数据日期的基差均值
    const basisSamples: number[] = [];
    for (const r of fredRows.slice(-5)) {
      const yv = yahooMap.get(r.period);
      if (yv && yv > 0) basisSamples.push(r.val - yv);
    }
    const avgBasis = basisSamples.length > 0
      ? basisSamples.reduce((a, v) => a + v, 0) / basisSamples.length
      : -4.5; // 当前市场经验值：期货比现货高约 $4-5
    console.log(`[窗口] Yahoo 基差样本: ${basisSamples.map(v => v.toFixed(2)).join(",")} → 均值=${avgBasis.toFixed(2)}`);
    // 用基差修正后的 Yahoo 价格填充 FRED 未覆盖的窗口日期
    for (const r of yahooBzRes.value) {
      if (r.period >= windowStart && !brentMap.has(r.period)) {
        const adjusted = +(r.val + avgBasis).toFixed(2);
        if (adjusted > 0) { brentMap.set(r.period, adjusted); }
      }
    }
  } else console.warn("[窗口] Yahoo BZ=F 失败:", (yahooBzRes.reason as Error)?.message);

  // ④ EIA 直连兜底（仅 FRED+AV+Yahoo 均失败时）
  if (brentMap.size === 0) {
    console.warn("[窗口] 所有现货源失败，降级 EIA 直连");
    const p = new URLSearchParams({
      api_key: EIA_KEY, frequency: "daily",
      "data[0]": "value", "sort[0][column]": "period", "sort[0][direction]": "asc", length: "15",
      "facets[product][]": "EPCBRENT", start: windowStart, end: todayStr,
    }).toString();
    const eiaRes = await fetch(`${EIA_BASE}?${p}`, { signal: AbortSignal.timeout(12000) })
      .then(r => r.json()).catch(() => null);
    for (const r of eiaRes?.response?.data ?? []) {
      const v = parseFloat(r.value); if (v > 0) brentMap.set(r.period, v);
    }
  }
  // 新周期无历史数据兜底：用实时Brent现货填充整个窗口（解决FRED/Yahoo/EIA均滞后时新周期首日无数据）
  if (brentMap.size === 0 && realtimeBrent > 0) {
    const tmpCur = new Date(windowStart + "T12:00:00Z");
    const tmpToday = new Date(todayStr + "T12:00:00Z");
    const tmpDays: string[] = [];
    while (tmpCur <= tmpToday) {
      const ds = tmpCur.toISOString().slice(0, 10);
      if (isWorkday(ds)) tmpDays.push(ds);
      tmpCur.setUTCDate(tmpCur.getUTCDate() + 1);
    }
    const tmpSpot = +(realtimeBrent - 4.5).toFixed(2); // 期货→现货经验基差，与下方 rtBasis 兜底一致
    for (const d of tmpDays.slice(0, 10)) {
      if (!brentMap.has(d)) brentMap.set(d, tmpSpot);
    }
    if (brentMap.size > 0) console.log(`[窗口] 新周期无历史现货数据，用实时Brent≈$${tmpSpot} 填充 ${brentMap.size} 天`);
  }
  if (brentMap.size === 0) throw new Error("所有数据源均失败，无法获取 Brent 窗口数据");

  // ── 2. 枚举窗口工作日（含今天），用实时盘价（基差修正）填充当日缺失 ─────
  //  窗口起点 = 调价日次日（nextWorkday(lastAdjustDate)），终点 = 今天（含）
  //  这样新周期首日即可开始累计，避免窗口为空时降级到14天回退窗口（会混入调价前数据）
  ...
  // 实时 Brent 也做基差修正（realtimeBrent 来自 CNBC/Yahoo 期货）
  // 取基差均值（若 fredRows 有足够数据），否则用经验值 -4.5
  const fredRecentVals = fredRows.slice(-3);
  let rtBasis = -4.5;
  if (yahooBzRes.status === "fulfilled" && fredRecentVals.length > 0) {
    const yahooMap2 = new Map((yahooBzRes.value ?? []).map((r: {period: string; val: number}) => [r.period, r.val]));
    const bs = fredRecentVals.map(r => r.val - (yahooMap2.get(r.period) ?? r.val)).filter(v => v !== 0);
    if (bs.length > 0) rtBasis = bs.reduce((a, v) => a + v, 0) / bs.length;
  }
  const realtimeBrentSpot = realtimeBrent > 0 ? +(realtimeBrent + rtBasis).toFixed(2) : 0;

  let fillCount = 0;
  for (const day of effectiveDays) {
    if (!brentMap.has(day) && realtimeBrentSpot > 0) { brentMap.set(day, realtimeBrentSpot); fillCount++; }
  }
  if (fillCount > 0) console.log(`[窗口] 实时现货填充 ${fillCount} 天（Brent期货$${realtimeBrent}→现货≈$${realtimeBrentSpot}，基差${rtBasis.toFixed(2)}）`);

  // ── 3. 计算三品种窗口均价（发改委口径：布伦特×40% + 迪拜×30% + 米纳斯×30%）───
  //  迪拜现货 ≈ 布伦特 - 8.5（当前 Brent/Dubai 价差，普氏实测）
  //  米纳斯现货 = 迪拜 - 3.2（迪拜/米纳斯民间拟合公式，价差稳定在 $3.1~3.4）
  ...
  const avgDubai = +(avgBrent - 8.5).toFixed(2);  // Brent-Dubai 现货利差（当前市场约 $8-9）
  const avgMinas = +(avgDubai - 3.2).toFixed(2);  // Dubai-Minas 利差（$3.1~3.4，取中值3.2）
  // 发改委一揽子 = 布伦特×40% + 迪拜×30% + 米纳斯×30%
  const avg10d   = +((avgBrent * 4 + avgDubai * 3 + avgMinas * 3) / 10).toFixed(2);
  const avgWti   = avgMinas; // 兼容旧接口：avgWti 字段改为返回 Minas 值
  ...
  console.log(`[窗口] Brent 现货明细: ${detail}`);
  console.log(`[窗口] 三品种均价: 布伦特(普氏)=$${avgBrent}(${days}天) 迪拜≈$${avgDubai}(Brent-8.5) 米纳斯≈$${avgMinas}(Dubai-3.2) 一揽子=$${avg10d} ${startDate}→${dataDate}`);
...
serve(async (req: Request): Promise<Response> => {
  ...
  return new Response(JSON.stringify({
    ...
    data: {
      ...
      avg10d: effectiveAvg10d, lastCycleAvg, lastAdjustDate,
