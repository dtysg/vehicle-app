...
export default function MusicPlayer() {
  ...
  // 循环模式：0=不循环 1=单曲循环 2=列表循环（默认列表循环）
  const [loopMode, setLoopMode] = useState<0 | 1 | 2>(2);
  const loopModeRef = useRef<0 | 1 | 2>(2);
  useEffect(() => { loopModeRef.current = loopMode; }, [loopMode]);
  const cycleLoopMode = useCallback(() => {
    setLoopMode(prev => { const n = ((prev + 1) % 3) as 0 | 1 | 2; loopModeRef.current = n; return n; });
  }, []);
  ...
  useEffect(() => {
    ...
    if (idx < 0) {
      // 非队列模式：列表循环=重播单曲，不循环=停止
      if (loop === 2) { try { player.seekTo(0); player.play(); } catch { /* ignore */ } }
      return;
    }
    // 队列模式
    const nextIdx = idx + 1;
    if (nextIdx < list.length) {
      ...
      // 列表循环：播完后从第一首重新开始
      setFavQueueIdx(0);
      playTrackFromFav(list[0], 0);
    } else {
      setFavQueueIdx(-1); // 不循环：停止
    }
              ...
              {favorites.length === 0 ? (
                ...
                <>
                  <View style={styles.favHeader}>
                    ...
                    {/* 循环模式切换 */}
                    <Pressable onPress={cycleLoopMode} hitSlop={10}
                      style={{ marginLeft: 'auto' as never, flexDirection: 'row', alignItems: 'center', gap: 3,
                        backgroundColor: loopMode > 0 ? 'rgba(96,165,250,0.15)' : 'rgba(255,255,255,0.05)',
                        borderRadius: 6, paddingHorizontal: 7, paddingVertical: 3,
                        borderWidth: 1, borderColor: loopMode > 0 ? 'rgba(96,165,250,0.35)' : 'rgba(255,255,255,0.1)' }}>
                      {loopMode === 1
                        ? <Repeat1 size={11} color="#60A5FA" />
                        : <Repeat size={11} color={loopMode === 2 ? '#60A5FA' : 'rgba(255,255,255,0.3)'} />}
                      <Text style={{ fontSize: 9, fontWeight: '700',
                        color: loopMode > 0 ? '#60A5FA' : 'rgba(255,255,255,0.3)' }}>
                        {loopMode === 0 ? '不循环' : loopMode === 1 ? '单曲' : '列表'}
                      </Text>
                    </Pressable>
                    {favQueueIdx >= 0 && (
